...
先考虑低噪声区 $\sigma \rightarrow 0$ ，对应高隐私预算和高模型效用性的情况。

为了理解上式中的和是由哪一项主导，这里显式写出真实采样向量 $x$ 处的评估指数与任意虚假采样向量 $x' \neq x$ 处评估指数之间的差距：

上式中指数部分为：

$$\frac{2 \langle \tilde{x}, y \rangle - \parallel \tilde{x} \parallel^2_2}{2 \sigma^2}$$

对于真实采样向量，将 $y = x + \sigma w$ 带入得：

$$\frac{2 \langle x, x + \sigma w \rangle - \parallel x \parallel^2_2}{2 \sigma^2}$$

对于虚假采样向量，将 $y = x + \sigma w$ 带入得：

$$\frac{2 \langle \tilde{x}, x + \sigma w \rangle - \parallel \tilde{x} \parallel^2_2}{2 \sigma^2}$$

两者相减得：

$$\frac{2 \langle x, x + \sigma w \rangle - \parallel x \parallel^2_2}{2 \sigma^2} - \frac{2 \langle \tilde{x}, x + \sigma w \rangle - \parallel \tilde{x} \parallel^2_2}{2 \sigma^2}$$

$$\frac{2 \langle x, x \rangle + 2\sigma \langle x, w \rangle - \parallel x \parallel^2_2}{2 \sigma^2} - \frac{2 \langle \tilde{x}, x \rangle + 2\sigma \langle \tilde{x}, w \rangle - \parallel \tilde{x} \parallel^2_2}{2 \sigma^2}$$

$$\frac{2\parallel x \parallel^2_2 + 2\sigma \langle x, w \rangle - \parallel x \parallel^2_2 - 2 \langle \tilde{x}, x \rangle - 2\sigma \langle \tilde{x}, w \rangle + \parallel \tilde{x} \parallel^2_2}{2 \sigma^2}$$

$$\frac{\parallel x \parallel^2_2 + \parallel \tilde{x} \parallel^2_2 - 2 \langle \tilde{x}, x \rangle + 2\sigma \langle x, w \rangle  - 2\sigma \langle \tilde{x}, w \rangle}{2 \sigma^2}$$

$$\frac{\parallel x - \tilde{x} \parallel^2_2 + 2\sigma \langle x - \tilde{x}, w \rangle}{2 \sigma^2}$$

$$\frac{\parallel x - \tilde{x} \parallel^2_2}{2 \sigma^2} + \frac{\langle x - \tilde{x}, w \rangle}{\sigma}$$
